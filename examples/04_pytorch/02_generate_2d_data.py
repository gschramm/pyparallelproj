...
image = cp.array(nii.get_fdata(), dtype=cp.float32)
...
image = image[:, :, start_sl:(start_sl + 1)]
...
attenuation_image = (0.01 * (image > 0)).astype(cp.float32)
...
res_model = resolution_models.GaussianImageBasedResolutionModel(
    image_shape, tuple(fwhm_mm_data / (2.35 * x) for x in voxel_size), cp, ndi)
...
emission_volume = cp.where(image > 0)[0].shape[0] * np.prod(voxel_size)
...
# generate noisy data
data = cp.random.poisson(image_fwd + contamination).astype(cp.uint16)
...
res_model_recon = resolution_models.GaussianImageBasedResolutionModel(
    image_shape, tuple(fwhm_mm_recon / (2.35 * x) for x in voxel_size), cp,
    ndi)
    ...
    json.dump(
        {
            ...
            for i in [
                ...
                "sim_number", "start_sl", "voxel_size", "trues_per_volume",
                "seed", "fwhm_mm_data", "fwhm_mm_recon"
            ]
        }, f)
